...
export function ScenarioSimulator({ wireframe = false, positionId }: ScenarioSimulatorProps) {
      ...
      <div className="bg-white border border-[#CCCCCC] rounded-lg p-4 flex flex-col h-full">
        ...
        <div className="mb-3">
          <div className="h-7 border border-[#CCCCCC] rounded bg-[#F5F5F5] flex items-center px-2">
            ...
            <span className="text-[9px] text-[#999999]">已选中：标的名称 (代码) CNY</span>
          </div>
          <div className="text-[8px] text-[#BBBBBB] mt-0.5">可搜索下拉，金额后缀币种简写</div>
        </div>
  ...
  return (
    <div className="bg-white rounded-xl border border-[#E8ECF0] p-3 shadow-sm flex flex-col h-full">
      <div className="text-xs font-semibold text-[#0D1117] mb-2">持仓盈亏情景模拟</div>
      ...
      <div className="mb-4 relative" ref={containerRef}>
        ...
        >
          ...
          <input
            ...
            value={open ? search : `${selected.underlying} (${selected.code}) ${selected.currency}`}
            ...
            {filtered.length === 0 ? (
              ...
              filtered.map((p) => (
                ...
                >
                  ...
                  <span className="text-[#9CA3AF] ml-auto text-[10px]">{p.currency} · 到期 {p.expiryDate}</span>
                </div>
              ))
            )}
          </div>
      ...
      {/* 固定模式：仅显示当前标的 */}
      {fixedId && (
        <div className="mb-3 flex items-center gap-1.5 px-2.5 py-1.5 bg-[#F9FAFB] rounded-lg border border-[#E5E7EB]">
          <span className="text-[#374151] text-[10px] font-medium truncate">{selected.underlying}</span>
          <span className="text-[#9CA3AF] text-[10px] flex-shrink-0">({selected.code})</span>
          <span className="text-[#9CA3AF] text-[10px] ml-auto">{selected.currency}</span>
        </div>
      )}
      ...
      <div className="mb-3">
        <div className="flex justify-between items-center mb-1.5">
          <span className="text-[10px] text-[#6B7280]">标的价格变化</span>
          <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${sliderValue > 0 ? 'text-[#E53935] bg-[#FFF1F0]' : sliderValue < 0 ? 'text-[#059669] bg-[#F0FDF4]' : 'text-[#6B7280] bg-[#F3F4F6]'}`}>
            {sliderValue > 0 ? '+' : ''}{sliderValue}%
          </span>
        </div>
        <div className="flex items-center gap-1.5">
          <button onClick={() => adjustByOne(-1)} disabled={sliderValue <= -100}
            className="w-6 h-6 flex items-center justify-center rounded border border-[#E5E7EB] bg-[#F9FAFB] hover:bg-[#F3F4F6] text-[#374151] disabled:opacity-30 disabled:cursor-not-allowed flex-shrink-0">
            <Minus size={10} />
          </button>
          <div className="flex-1 relative"><input type="range" min={-100} max={100} step={1} value={sliderValue}
              onChange={(e) => setSliderValue(Number(e.target.value))}
              className="w-full h-1.5 appearance-none rounded-full cursor-pointer"
              style={{ background: `linear-gradient(to right, #059669 0%, #E5E7EB 50%, #E53935 100%)` }} /></div>
          <button onClick={() => adjustByOne(1)} disabled={sliderValue >= 100}
            className="w-6 h-6 flex items-center justify-center rounded border border-[#E5E7EB] bg-[#F9FAFB] hover:bg-[#F3F4F6] text-[#374151] disabled:opacity-30 disabled:cursor-not-allowed flex-shrink-0">
            <Plus size={10} />
          </button>
        </div>
        <div className="flex justify-between text-[8px] text-[#9CA3AF] mt-0.5"><span>-100%</span><span>0%</span><span>+100%</span></div>
        <div className="mt-1.5 flex items-center gap-1 text-[10px]">
          <span className="text-[#6B7280]">模拟</span>
          <span className="font-semibold text-[#0D1117]">{(selected.currentPrice * (1 + sliderValue / 100)).toFixed(2)} {cur}</span>
          <span className={`font-bold ${sliderPnl >= 0 ? 'text-[#E53935]' : 'text-[#059669]'}`}>{formatAmount(convertCurrency(sliderPnl, cur), cur, false)}</span>
        </div>
      </div>

      {/* 固定场景模拟 */}
      <div>
        <div className="text-[9px] font-medium text-[#6B7280] mb-1.5">固定场景模拟（与拉杆独立）</div>
        <div className="space-y-0.5">
          {FIXED_POINTS.map((pct) => {
            const pnl = calcPnl(selected, pct);
            const converted = convertCurrency(pnl, cur);
            return (
              <div key={pct} className={`flex items-center justify-between rounded px-2 py-1 ${pct === 0 ? 'bg-[#EFF6FF]' : 'bg-[#F9FAFB]'}`}>
                <span className={`text-[10px] font-medium ${pct === 0 ? 'text-[#1677FF]' : pct > 0 ? 'text-[#E53935]' : 'text-[#059669]'}`}>
                  {pct === 0 ? '当前' : `${pct > 0 ? '+' : ''}${(pct * 100).toFixed(0)}%`}
                </span>
                <span className={`text-[10px] font-semibold ${pnl >= 0 ? 'text-[#E53935]' : 'text-[#059669]'}`}>{formatAmount(converted, cur, false)}</span>
              </div>
            );
          })}
          {sliderValue !== 0 && (
            <div className="flex items-center justify-between rounded px-2 py-1 bg-[#FFFBEB]">
              <span className="text-[10px] font-medium text-[#F59E0B]">{sliderValue > 0 ? '+' : ''}{sliderValue}%</span>
              <span className={`text-[10px] font-semibold ${sliderPnl >= 0 ? 'text-[#E53935]' : 'text-[#059669]'}`}>{formatAmount(convertCurrency(sliderPnl, cur), cur, false)}</span>
            </div>
          )}
        </div>
      </div>

      <div className="text-[8px] text-[#9CA3AF] leading-relaxed text-center mt-auto pt-2">
        模拟结果仅供参考，实际盈亏以行权/平仓时的市场价格和交易规则为准
      </div>
    </div>
  );
}
